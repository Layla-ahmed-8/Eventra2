...
export default function AttendeeLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-pink-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900 flex">
      {/* Sidebar - Desktop */}
      <aside
      ...
      >
        ...
        <div className="h-24 flex items-center justify-between px-6 border-b border-purple-100/50 dark:border-slate-800/50 bg-gradient-to-r from-white/80 to-purple-50/50 dark:from-slate-900/80 dark:to-purple-950/20">
          {sidebarOpen ? (
            <Link to="/app/discover" className="flex items-center min-w-0 animate-in fade-in duration-700">
              <Logo variant="horizontal" className="h-9 w-auto" />
            </Link>
          ) : (
            <Link to="/app/discover" className="flex items-center justify-center w-full animate-in zoom-in duration-700">
            ...
            return (
              <Link
                ...
                className={`relative flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 group hover:scale-[1.02] ${
                  isActive
                    ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg shadow-purple-500/30 translate-x-1'
                    : 'text-slate-600 dark:text-slate-300 hover:bg-gradient-to-r from-purple-50/50 to-pink-50/50 dark:hover:bg-purple-900/20 hover:translate-x-1'
                }`}
              ...
              >
                ...
                {sidebarOpen && item.path === '/app/notifications' && unreadCount > 0 && (
        ...
        <div className="p-5 space-y-4 border-t border-purple-100/50 dark:border-slate-800/50 bg-gradient-to-t from-purple-50/40 to-white/80 dark:from-purple-950/20 dark:to-slate-900/80">
          ...
          <Link
            to="/app/profile"
            className={`flex items-center gap-4 p-4 rounded-2xl hover:bg-gradient-to-r from-purple-50/60 to-pink-50/60 dark:hover:bg-purple-900/20 transition-all group ${
          ...
          >
            <div className="relative flex-shrink-0">
              <img
                ...
                className="w-14 h-14 rounded-2xl ring-4 ring-purple-400/20 dark:ring-purple-500/20 group-hover:ring-purple-400/40 dark:group-hover:ring-purple-500/40 transition-all object-cover"
              />
              <div className="absolute -bottom-2 -right-2 w-7 h-7 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center text-xs text-white font-black shadow-xl shadow-purple-500/30">
                {currentUser?.level}
              </div>
            </div>
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Level {currentUser?.level}</p>
                <WalletBalanceBadge colorClass="text-purple-600 dark:text-purple-400" />
              </div>
            )}
          </Link>
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 hover:scale-[1.01] ${
                      isActive
                        ...
                        : 'text-slate-700 dark:text-slate-300 hover:bg-gradient-to-r from-purple-50/60 to-pink-50/60 dark:hover:bg-purple-900/20'
                    }`}
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-7 border-t border-purple-100/60 dark:border-slate-800/60 bg-gradient-to-t from-purple-50/60 to-white/80 dark:from-purple-950/20 dark:to-slate-900/80 backdrop-blur-xl">
              <div className="flex items-center gap-4 mb-6">
                <img
                  src={currentUser?.avatar || 'https://i.pravatar.cc/150?img=33'}
                  alt={currentUser?.name}
                  className="w-14 h-14 rounded-2xl object-cover ring-4 ring-purple-400/20 dark:ring-purple-500/20"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-lg font-bold text-slate-900 dark:text-white truncate">{currentUser?.name}</p>
                  <p className="text-xs font-black text-slate-500 dark:text-slate-400">Level {currentUser?.level} · {currentUser?.xp} XP</p>
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <div className="lg:hidden h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border-b border-purple-100/60 dark:border-slate-800/60 flex items-center justify-between px-6 sticky top-0 z-40 shadow-lg shadow-purple-900/5 dark:shadow-black/30">
          ...
          <div className="flex items-center gap-3">
            <Link to="/app/notifications" className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-slate-100 to-purple-100 dark:from-slate-800 dark:to-purple-900/30 flex items-center justify-center border border-purple-200/60 dark:border-slate-700 hover:scale-105 transition-transform">
            ...
            <Link to="/app/profile">
              <img
                src={currentUser?.avatar || 'https://i.pravatar.cc/150?img=33'}
                alt={currentUser?.name}
                className="w-12 h-12 rounded-2xl object-cover ring-4 ring-purple-400/20 dark:ring-purple-500/20 hover:scale-105 transition-transform"
              />
            </Link>
          </div>
        </div>
