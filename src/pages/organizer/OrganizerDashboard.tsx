...
import {
  ...
  Eye, Clock, CheckCircle2, AlertCircle, Target, Activity,
  Star, ChevronRight, Flame
} from 'lucide-react';
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="surface-panel p-5">
            <h3 className="text-h3 font-bold text-foreground mb-4">Overall Fill Rate</h3>
            <div className="flex items-center gap-4">
              <div className="relative flex-shrink-0">
                <RadialProgress value={87} size={72} stroke={7} color="#7C5CFF" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-body-sm font-bold text-foreground">87%</span>
                </div>
              </div>
              <div className="space-y-2 flex-1">
                {[
                  { label: 'Jazz Night', val: 71, color: '#7C5CFF' },
                  { label: 'AI Summit', val: 77, color: '#00D4FF' },
                  { label: 'Food Fest', val: 65, color: '#FF9B3D' },
                ].map(e => (
                  <div key={e.label}>
                    <div className="flex justify-between text-caption mb-0.5">
                      <span className="text-muted-foreground">{e.label}</span>
                      <span className="font-semibold text-foreground">{e.val}%</span>
                    </div>
                    <div className="h-1 bg-muted rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${e.val}%`, background: e.color }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* This month */}
          <div className="surface-panel p-5">
            <h3 className="text-h3 font-bold text-foreground mb-4">This Month</h3>
            <div className="space-y-3">
              {[
                { label: 'Page Views', value: '12.4K', icon: Eye },
                { label: 'Conversion Rate', value: '24%', icon: Target },
                { label: 'Avg Ticket Price', value: 'EGP 125', icon: DollarSign },
                { label: 'Avg Response Time', value: '1.2 hrs', icon: Clock },
              ].map(s => (
          ...
          {/* Quick actions */}
          <div className="surface-panel p-5">
            <h3 className="text-h3 font-bold text-foreground mb-3">Quick Actions</h3>
            <div className="space-y-2">
              <Link to="/organizer/events/create" className="btn-primary w-full text-body-sm">
                <Plus className="w-4 h-4" /> Create Event
              </Link>
              <Link to="/organizer/messages" className="btn-secondary w-full text-body-sm">
                <MessageSquare className="w-4 h-4" /> Message Attendees
              </Link>
              <Link to="/organizer/analytics" className="btn-secondary w-full text-body-sm">
